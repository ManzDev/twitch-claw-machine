(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))a(o);new MutationObserver(o=>{for(const i of o)if(i.type==="childList")for(const s of i.addedNodes)s.tagName==="LINK"&&s.rel==="modulepreload"&&a(s)}).observe(document,{childList:!0,subtree:!0});function e(o){const i={};return o.integrity&&(i.integrity=o.integrity),o.referrerpolicy&&(i.referrerPolicy=o.referrerpolicy),o.crossorigin==="use-credentials"?i.credentials="include":o.crossorigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function a(o){if(o.ep)return;o.ep=!0;const i=e(o);fetch(o.href,i)}})();class n extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}static get styles(){return`
      :host {
        display: grid;
        place-items: center;
      }
    ...
    `}init(){const t=this.classList.contains("chosen"),e=z();if(this.putGift(e),!t){const i=-50+Math.floor(Math.random()*100);this.style.setProperty("--rotate",`${i}deg`)}const a=Math.floor(Math.random()*w.length),o=w[a];this.style.setProperty("--color",o)}async connectedCallback(){this.render(),await this.init()}putGift(t){this.shadowRoot.querySelector(".ball").insertAdjacentHTML("afterbegin",`<img class="gift" src="${t}" alt="Gift">`)}render(){this.shadowRoot.innerHTML=`
    ...
    `}}customElements.define("made-box",F);const y=2,E=-60,L=60,k=new Audio("sounds/rolling.mp3"),C=new Audio("sounds/beepbox.mp3"),T=()=>{k.currentTime=0,k.play()},A=()=>{C.currentTime=0,C.play()};class b extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}init(){this.claw={x:0},globalThis.addEventListener("keydown",t=>this.onKeydown(t)),globalThis.addEventListener("keyup",t=>this.onKeyup(t)),document.addEventListener("transferball",t=>this.winBall(t.detail))}winBall(t){T(),this.shadowRoot.querySelector(".output-slot").appendChild(t),setTimeout(()=>t.classList.add("outside"),750),setTimeout(()=>{t.classList.add("floating"),A()},1500)}update(){this.style.setProperty("--claw-x",`${this.claw.x}px`)}onKeydown(t){const{key:e}=t;if(!this.shadowRoot.querySelector("crystal-cube").isClawBlocked()){if(e==="ArrowLeft"){const o=new CustomEvent("SPIN_CLAW",{detail:"left"});document.dispatchEvent(o),this.claw.x=Math.max(E,this.claw.x-y),this.update()}else if(e==="ArrowRight"){const o=new CustomEvent("SPIN_CLAW",{detail:"right"});document.dispatchEvent(o),this.claw.x=Math.min(L,this.claw.x+y),this.update()}else if(e===" "){const o=new CustomEvent("PRESS_BUTTON",{detail:{}});document.dispatchEvent(o),this.shadowRoot.querySelector(".output-slot").innerHTML=""}}}onKeyup(t){const{key:e}=t;if(e==="ArrowLeft"){const a=new CustomEvent("SPIN_CLAW",{detail:"left"});document.dispatchEvent(a)}else if(e==="ArrowRight"){const a=new CustomEvent("SPIN_CLAW",{detail:"right"});document.dispatchEvent(a)}else if(e===" "){const a=new CustomEvent("RELEASE_BUTTON",{detail:{}});document.dispatchEvent(a)}}static get styles(){return`
      :host {
        --width: 250px;
